...
resource "aws_security_group" "demo_webservers"{
  tags = {
    Name = "${var.ENVIRONMENT}-demo-webservers"
  }
...
resource "aws_launch_configuration" "launch_config_webserver" {
  ...
  security_groups = [aws_security_group.demo_webservers.id]
...
resource "aws_autoscaling_group" "demo_webserver" {
...
resource "aws_lb" "demo-load-balancer" {
  name               = "${var.ENVIRONMENT}-demo-lb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.demo_webservers_alb.id]
...
resource "aws_lb_listener" "webserver_listner" {
  load_balancer_arn = aws_lb.demo-load-balancer.arn
  port              = "80"
  protocol          = "HTTP"
